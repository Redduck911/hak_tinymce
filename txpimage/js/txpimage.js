var TxpImageDialog = {
	...
	init : function(ed) {
	    ...
	    if (ed.getParam("txpimage_constrain_proportions", true)) {
	...
	if (tinyMCEPopup.getParam("accessibility_warnings", 1)) {
	    if (!f.alt.value) {
		tinyMCEPopup.confirm(tinyMCEPopup.getLang('txpimage_dlg.missing_alt'), function(s) {
		    if (s) {
			t.insertAndClose();
		    }
		});
	...
	var dom = tinyMCEPopup.dom, lst = dom.get(id), v = tinyMCEPopup.getParam('theme_advanced_styles'), cl;
    ...
    insertImage:function (obj, imgHash) {
	var f = document.forms[0], nl = f.elements;
	var needDecode = ['title', 'alt'];
	tinymce.each(imgHash, function (v, k) {
...
TxpImageDialog.preInit();
tinyMCEPopup.onInit.add(TxpImageDialog.init, TxpImageDialog);
